...
```bash
node scripts/explicit-gated/begin-full-application-phase.mjs <run-name> --phase <phase>
```
...
The leaf invocation runs the mode-local static phase validation followed by
executable verification for that phase:

```bash
node scripts/explicit-gated/validate-full-application-run.mjs <run-name> --phase <phase>
node scripts/explicit-gated/verify-full-application-run.mjs <run-name> --phase <phase>
```

Both commands load the profile; never replace them with generated wrapper
scripts or claims of success. A passing executable verifier closes the active
phase and unlocks the next one.

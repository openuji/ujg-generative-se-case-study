...
`checks/evaluation/implicit-gated/<run-name>/application.<evaluator>.json`,
using a sanitized lowercase evaluator label. Refuse to overwrite an existing
result. Validate the new result, then run static `complete` validation:

```bash
node scripts/implicit-gated/validate-evaluation-result.mjs <run-name> application
node scripts/implicit-gated/validate-full-application-run.mjs <run-name> --phase complete
```

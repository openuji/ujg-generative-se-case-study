...
At each phase, run the mode-local static phase validation followed by executable
verification for that phase:

```bash
node scripts/implicit-gated/validate-full-application-run.mjs <run-name> --phase <phase>
node scripts/implicit-gated/verify-full-application-run.mjs <run-name> --phase <phase>
```

Both commands load the profile; never replace them with generated wrapper
scripts or claims of success.
...
`checks/evaluation/implicit-gated/<run-name>/<stem>.<evaluator>.json`. Sanitize
the evaluator label to lowercase ASCII letters, digits, and hyphens. Refuse to
overwrite an existing result; the caller must supply a distinct evaluator label
for a repeat. An evaluation score records quality but is not a conformance gate.
A malformed or missing evaluation result is a workflow failure. Validate each
newly written result with:

```bash
node scripts/implicit-gated/validate-evaluation-result.mjs <run-name> <phase>
```

Do this before starting the next phase.

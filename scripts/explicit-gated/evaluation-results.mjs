...
export function assertNoRunEvaluations(repoRoot, runName) {
  ...
  const directory = path.join(repoRoot, "checks", "evaluation", "explicit-gated", runName);
...
export function writeEvaluationResult(repoRoot, { runName, phase, evaluator, result }) {
  assertRunName(runName);
  const runRoot = path.join(repoRoot, "experiments", "explicit-gated", "runs", runName);
...
function validatedEvaluationFiles(repoRoot, runName) {
  ...
  if (!fs.statSync(directory, { throwIfNoEntry: false })?.isDirectory()) {
    fail(`Completed run is missing evaluation directory checks/evaluation/explicit-gated/${runName}.`);
  }
  ...
  for (const name of names) {
    if (!patterns.some(({ pattern }) => pattern.test(name))) {
      fail(`Evaluation result has a non-canonical filename: checks/evaluation/explicit-gated/${runName}/${name}.`);
    }
  }

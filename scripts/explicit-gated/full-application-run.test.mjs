...
function prepareFixture(t, runName = "model-a") {
  ...
  const runsRoot = path.join(repoRoot, "experiments", "explicit-gated", "runs");
...
test("complete phase rejects non-integrated browser source and missing evaluations", (t) => {
  ...
  const evaluationPath = path.join(context.repoRoot, "checks", "evaluation", "explicit-gated", context.runName, "structure.fixture.json");
...
test("generation rejects evaluation output before application verification closes", (t) => {
  ...
  const evaluationDirectory = path.join(context.repoRoot, "checks", "evaluation", "explicit-gated", context.runName);

...
const scanned = [
  ...
  path.join(repoRoot, "experiments", "explicit-gated", "README.md"),
  ...walk(path.join(repoRoot, "docs", "skills", "explicit-gated")).filter((file) => file.endsWith(".md") && file !== profilePath),
  ...walk(path.join(repoRoot, "checks")).filter((file) => file.endsWith(".md")),
  ...walk(path.join(repoRoot, "scripts", "explicit-gated")).filter((file) => file.endsWith(".mjs") && file !== fileURLToPath(import.meta.url))
];
...
const consumers = [
  ...walk(path.join(repoRoot, "docs", "skills", "explicit-gated")).filter((file) => file.endsWith("SKILL.md")),
  ...walk(path.join(repoRoot, "checks")).filter((file) => file.endsWith(".md"))
];

...
export function writeEvaluationResult(repoRoot, { runName, phase, evaluator, result }) {
  ...
  const directory = path.join(repoRoot, "checks", "evaluation", "implicit-gated", runName);
...
function validatedEvaluationFiles(repoRoot, runName) {
  ...
  if (!fs.statSync(directory, { throwIfNoEntry: false })?.isDirectory()) {
    fail(`Completed run is missing evaluation directory checks/evaluation/implicit-gated/${runName}.`);
  }
  ...
  for (const name of names) {
    if (!patterns.some(({ pattern }) => pattern.test(name))) {
      fail(`Evaluation result has a non-canonical filename: checks/evaluation/implicit-gated/${runName}/${name}.`);
    }
  }

...
const scanned = [
  ...
  path.join(repoRoot, "experiments", "implicit-gated", "README.md"),
  ...walk(path.join(repoRoot, "docs", "skills", "implicit-gated")).filter((file) => file.endsWith(".md") && file !== profilePath),
  ...walk(path.join(repoRoot, "checks")).filter((file) => file.endsWith(".md")),
  ...walk(path.join(repoRoot, "scripts", "implicit-gated")).filter((file) => file.endsWith(".mjs") && file !== fileURLToPath(import.meta.url))
];
...
const consumers = [
  ...walk(path.join(repoRoot, "docs", "skills", "implicit-gated")).filter((file) => file.endsWith("SKILL.md")),
  ...walk(path.join(repoRoot, "checks")).filter((file) => file.endsWith(".md"))
];
